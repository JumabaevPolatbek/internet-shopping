...
export function PageProduct(props:Product){
    const {name,images,description,category,quantity,discount,price,id}=props
    ...
    const handleDelProduct=async (id:number,name:string)=> await delProduct(id)
        .unwrap()
        .then((response)=>
            toast.success(`${name} продукт успешьно удалень`,{
                position: "top-right",
                autoClose: 3000,
        ...
        <TableRow >
            ...
            <TableCell>
                <div className='flex items-center'>
                    ...
                    <Tooltip title="Изменение продукта" arrow>
                            <IconButton
                                color='secondary'
                        ...
                        <CustomizedDialogs open={open} setOpen={setOpen}>
                            <EditProduct product={props} setOpen={setOpen}/>
                        </CustomizedDialogs>
                    <Tooltip title="Delete Product" arrow>
                        <IconButton
                            color='primary'
                            onClick={()=>handleDelProduct(id,name)}
                        >
                            <DeleteIcon/>
                        </IconButton>
                    </Tooltip>
