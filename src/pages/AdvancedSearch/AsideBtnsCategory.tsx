...
export const AsideBtnsCategory = React.forwardRef<HTMLElement, Props>(
  (props, ref) => {
    const { category_id } = useAppSlector((state) => state.searchState);
    const dispatch = useDispatch();
    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      dispatch(addIdCategory(+(event.target as HTMLInputElement).value));
      if (props.type === " ") {
