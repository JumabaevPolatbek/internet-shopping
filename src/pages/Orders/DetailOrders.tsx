...
export function DetailOrders(props: ResponseOrders) {
  ...
  const handleDelOrder = async (order_id: number, user_id: number) =>
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        Продукты
        <IconButton onClick={() => setProducts((products) => !products)}>
          <AutoAwesomeMotionIcon />
        </IconButton>
        <CustomizedDialogs open={products} setOpen={setProducts}>
          {order_details?.map((product) => (
            <OrderProducts id={product.product_id} key={product.product_id} />
          ))}
        </CustomizedDialogs>
      </TableCell>
      ...
      <TableCell>
        <div className="flex items-center">
          ...
          <Tooltip title="Удалить заказ" arrow>
            <IconButton
              color="primary"
              onClick={() => handleDelOrder(user_id, id)}
            >
              <DeleteIcon />
            </IconButton>
          </Tooltip>
