...
export function AddOrderStatus({
	setOpen,
	type,
	id,
	name,
	...
	const [updateStatus, resultUpdate] =
		useUpdateOrderStatusMutation();
	...
	const btnSubmitAdd: SubmitHandler<
		Omit<ResponseOrderStatus, 'id'>
	> = async (data) =>
		await addStatus(data)
			...
			.then((response) => {
				toast.success(
					`${response.status} успешно добавлен!`,
					{
						position: 'top-right',
						autoClose: 3000,
	...
	> = async (data) =>
		await updateStatus({
			...
			.then((response) => {
				toast.success(
					`${response.status} успешно изменен!`,
					{
						position: 'top-right',
						autoClose: 3000,
						hideProgressBar: false,
	...
	const [age, setAge] = React.useState(data?.find(status=>status.id===id)?.status || '');
